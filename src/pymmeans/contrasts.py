...
def _contrast_one_family(
    ...
    correlation = None
    # the `mvt` alias was added in
    # (`_ADJUST_ALIASES = {"mvt": "dunnett"}`) but this
    # adjacent correlation-construction check still tested `adjust ==
    # "dunnett"` literally. Calling `pairs(em, adjust="mvt")` flowed
    # to the adjustments dispatcher with correlation=None, which then
    # raised "Dunnett requires correlation for k>1". Resolve the alias
    # locally so both names build the correlation matrix.
    # `dunnettx` is its own method
    # (R `.pdunnx`), correlation-free — do NOT build the correlation
    # matrix for it. Only `mvt` collapses to the dunnett code path.
    _adj_lower = adjust.lower()
    adj_canon = "dunnett" if _adj_lower == "mvt" else _adj_lower
    if adj_canon == "dunnett" and len(est) > 1:
...
def _contrast_result_from_emm(
    ...
    if df_method in ("satterthwaite", "kenward_roger"):
        # refusal is now / fall-through:
        # both Satt () and KR () carry a pickle-safe
        # `_satt_cache` populated by the original apply_* call, so the
        # propagation works post-pickle without `raw_result`. The
        # refusal only fires now if BOTH raw_result and the cache are
        # missing — usually means the EMM was hand-built or built
...
def contrast(
    ...
    # (): R's
    # ``contrast(em, interaction = c("pairwise", "consec"))`` builds
    # the Kronecker product of per-factor contrast matrices and
    ...
    # (): emm_options(adjust=...)
    # should propagate into contrast()'s default selection. The
    # fix wired it into pairs() but missed contrast(); the
    # named-method dispatch below now consults the option, and the
    # custom-contrast branch falls through to bonferroni (no change)
...
def effect_size(
    ...
    contrast) plus the user's pooled SD and its effective df.
    () adds the EMM input path, an ``edf`` argument,
    and the R-style ``SE`` / ``lower_cl`` / ``upper_cl`` columns derived
    from R's log-sigma delta-method approximation
    ...
    # (): R-style effect_size SE +
    # CI columns. R's eff_size.emmGrid uses the log-sigma delta
    # approximation:
    #
    # SE(d) = sqrt((SE_estimate / sigma)^2 + (d^2) / (2 * edf))
    #
...
def rbind(
    ...
    *results
    ...
    # () would silently drop every draw.
