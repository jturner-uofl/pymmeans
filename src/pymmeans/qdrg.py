...
def qdrg(
    ...
    *,
    ...
    # Accept three input styles for the RHS:
    #   - bare ``"g + x"`` (no tilde)
    #   - R-style RHS-only ``"~ g + x"`` (leading tilde)
    #   - full ``"y ~ g + x"`` (LHS + tilde + RHS)
    # Normalise to the bare-RHS form before synthesising a dummy LHS.
    needs_lhs = ("~" not in formula_s) or formula_s.lstrip().startswith("~")
    if needs_lhs:
        rhs = formula_s.lstrip().removeprefix("~").lstrip() if formula_s.lstrip().startswith("~") else formula_s
