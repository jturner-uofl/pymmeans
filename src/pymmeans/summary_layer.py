...
def _recompute_response_from_link(
    ...
    # (): one-sided ``side=`` should
    # leave the unconstrained endpoint at ±Inf on the response scale.
...
def summary(
    ...
    """
    # (): R `summary.emm_list` recurses
    # into each member and returns a named list. The Python analogue
    ...
    # (): the caller may override via
    # the new ``bias_adjust=`` and ``sigma=`` kwargs, mirroring R
    ...
    # (): R always runs the
    # hypothesis test on the LINK scale, even when display is on the
    ...
    if show_ci:
        ...
        if is_posterior:
            ...
            #
            # (): also rebuild when an
            # explicit multiplicity adjustment OR a one-sided ``side``
...
def confint(
    ...
    (): R's ``confint.emmGrid`` has a
    hard ``level = 0.95`` default that overrides any baked-in level
    on the EMM object — so ``confint(update(em, level=.99))`` returns
    a **95%** CI in R.

    (): pymmeans now matches R *strictly*
    — ``level=None`` resolves to **0.95**, independent of
    ``emm_options(level=...)``. Previously the fix honoured
    the option, but R does not (its ``confint.emmGrid`` formal is a
    bare ``level = 0.95`` with no option consultation). To globally
    raise the CI level via pymmeans, pass ``level=`` explicitly or
    call ``summary(obj, infer=(True, False), level=...)``, which
    *does* honour ``emm_options(level=...)``.

    (): ``bias_adjust=`` and ``sigma=``
    are forwarded to :func:`summary` and ultimately
...
def as_r_frame(
    ...
    (): ``EmmList`` is also accepted.
    Mirrors R ``as.data.frame.emm_list``: combines each member's
    summary into one DataFrame, padding non-shared columns with
    ``"."``, and emits R's UserWarning about combined results
    affecting adjusted P-values. Previously ``as_r_frame(EmmList)``
    crashed with ``AttributeError: 'EmmList' object has no
    attribute 'frame'``.

    (): the ``.set`` provenance column
    is now opt-in via ``include_set=True`` (default ``False``). R's
    ``as.data.frame.emm_list`` does NOT emit a ``.set`` column —
    members are just stacked. Strict R-parity is the new default; pass
