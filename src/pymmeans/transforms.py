...
def regrid_response(
    ...
    (): ``sigma`` may be a scalar OR a
    1-D ndarray broadcastable against the EMM rows (one element per
    row, R parity). Negative or non-finite values trigger a
    UserWarning and skip the bias adjustment.
    ...
    if kind == "emm":
        ...
        if bias_adjust:
            ...
            # (): R `regrid(em,
            # bias.adjust=TRUE)` builds the response-scale CI as
...
def regrid(
    ...
    """
    # (): R idiomatically uses
    # ``transform = NULL`` to mean "leave the scale alone". Accept
    # Python ``None`` as the equivalent so users porting R code don't
    # hit a NotImplementedError on a documented no-op.
