...
def joint_tests(
    ...
    **kwargs: Any,
    ...
    if isinstance(model, _EmmList):
        member = next(
            (m for m in model if hasattr(m, "model_info")),
            None,
        )
        if member is None:
            raise TypeError(
        ...
        # Re-dispatch with the unwrapped member. ``inference_kind``
        # propagates through EmmList members, so a posterior EmmList
        # hits the refusal below; a frequentist EmmList unwraps to a
        # plain EMMResult / ContrastResult and proceeds normally.
        model = member
