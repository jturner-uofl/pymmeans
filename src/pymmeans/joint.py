...
def joint_tests(
    ...
    **kwargs: Any,
    ...
    # Accept EmmList by unwrapping the first member that exposes a
    # ``model_info``. R ``emmeans::joint_tests`` accepts ``emm_list``
    # too (the result of ``emmeans(fit, pairwise ~ x)``); dispatch
    # before the per-member type check so the posterior-refusal
    # branch below sees the same ``inference_kind`` marker as a bare
    # EMMResult / ContrastResult.
    from pymmeans.contrasts import EmmList as _EmmList

    if isinstance(model, _EmmList):
        # Scan EVERY model_info-carrying member for a posterior
        # ``inference_kind`` marker. The earlier implementation
        # unwrapped only the FIRST such member, so a mixed
        # ``EmmList(freq_em, post_em)`` silently dispatched on the
        # frequentist member while the posterior member sat
        # unguarded — wrong by intent (any posterior content in the
        # container means Wald F / chi-squared is meaningless on the
        # whole bundle).
        members_with_info = [m for m in model if hasattr(m, "model_info")]
        if not members_with_info:
            raise TypeError(
                "joint_tests(EmmList) requires at least one member to "
                "expose a ``model_info``; got an EmmList containing "
                f"only {[type(m).__name__ for m in model]}."
            )
        posterior_idxs = [
            i for i, m in enumerate(members_with_info)
            if getattr(m, "inference_kind", "wald") == "posterior"
        ]
        if posterior_idxs:
            raise ValueError(
                "joint_tests() is not defined for posterior-derived "
                "EmmList containers. EmmList members at positions "
                f"{posterior_idxs} carry ``inference_kind='posterior'`` "
                "(from `from_pymc` / `posterior_emmeans`). The Wald "
                "F / chi-squared statistic assumes a frequentist "
                "sampling distribution of beta_hat; applying it to "
                "posterior covariances — even mixed with frequentist "
                "members — silently mixes two paradigms. For a "
                "Bayesian joint test, compute the posterior probability "
                "of the null contrast directly from the draws (e.g. via "
                "`posterior_emm_summary` on the linear combination of "
                "interest), or refit with a frequentist estimator "
                "before calling `joint_tests`."
            )
        # All members are frequentist; unwrap the first for the term-
        # structure walk below. (Every member shares the same
        # ``model_info`` and ``design_info`` because they came from
        # the same fit; picking the first is well-defined.)
        model = members_with_info[0]
    ...
    di = info.design_info
    # Pickling an EMMResult drops ``design_info`` (it carries
    # unpicklable patsy state); ``joint_tests`` needs ``design_info``
    # to walk the model's term structure. Deep inside the loop below
    # the missing attribute surfaces as ``AttributeError: 'NoneType'
    # object has no attribute 'terms'`` with no hint that pickling
    # was the cause. Detect early and steer the user.
    if di is None:
        raise ValueError(
            "joint_tests() requires the model's design_info to walk "
            "the term structure, but the input EMM/contrast/RefGrid "
            "has design_info=None. Most commonly this means the "
            "result was unpickled (pickling drops the unpicklable "
            "patsy design_info), or it was hand-built via ``emmobj()`` "
            "without an underlying fit.\n\n"
            "Fixes:\n"
            "  - Run joint_tests(fit) BEFORE pickling, and pickle the "
            "returned DataFrame instead of the EMM.\n"
            "  - Re-fit the original model in the current process and "
            "pass the fresh fit (or fresh emmeans(fit, ...) result) to "
            "joint_tests().\n"
            "  - For hand-built EMMs, joint_tests is not defined "
            "because there is no model term structure to traverse."
        )
