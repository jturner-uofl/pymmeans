...
def kenward_roger_vcov(
    ...
    # variance gap of ~2% (). The replacement matches
    # pbkrtest to ``atol≈5e-7`` on the full vcov diagonal — closing .
    #
...
def apply_kenward_roger(emm_or_contrast: Any) -> Any:
    ...
    .. note::
       **matches pbkrtest to atol≈5e-7.**
       The intercept-variance gap () was closed in
       by rewriting ``kenward_roger_vcov`` to implement
       pbkrtest's exact ``vcovAdj_internal`` algorithm. All entries
       of the KR-adjusted vcov now agree with ``pbkrtest::vcovAdj``
       to ``atol=4.76e-7`` on the canonical lme4 reference fit
       and to floating-point precision on sleepstudy.
