...
def from_predict(
    ...
    ... "age": np.random.randn(150),
    ... "y": np.random.randn(150),
    ... })
    >>> X = pd.get_dummies(df[["treatment", "age"]], drop_first=True)
    >>> gbm = GradientBoostingRegressor().fit(X, df["y"])
    >>>
    >>> def predict(d):
    ... X_new = pd.get_dummies(d[["treatment", "age"]], drop_first=True)
    ... return gbm.predict(X_new[X.columns])
    >>>
    >>> info = from_predict(predict, df, factors=["treatment"],
    ... numerics=["age"], response="y")
    """
    if numerics is None:
        factor_names = (
...
def ml_contrast(
    ...
    # ambiguous". The same gotcha bit `contrast()` historically
    # () and is back here in the ML adapter.
