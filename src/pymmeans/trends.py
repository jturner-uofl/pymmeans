...
def emtrends(
    ...
    delta_var
        ...
        supplied, overrides ``h``. ():
        when ``var`` has only one observed value, ``h`` defaulting
        silently to ``1e-5`` produced a spurious slope. R raises
        "Provide a nonzero value of 'delta.var'" in that case. We now
        mirror R: refuse with the same message unless ``delta_var``
        (or ``h``) was explicitly supplied.
    ...
    # (): refuse a degenerate numeric
    # predictor (zero range) unless the user explicitly supplied
    # ``delta_var=`` (or a custom ``h``). R raises the exact text
    # ``"Provide a nonzero value of 'delta.var'"`` in this case
    # (see emmeans/R/emtrends.R), and silently returning a slope
    # from a vacuous finite-difference would mislead users.
    # replace the previous string-match catch
    # (``if "delta_var" in str(e): raise``) with a structurally
    # separated try-block. Numeric coercion errors are caught and
    # ignored (let downstream code raise its own error); the
    # deliberately-raised "Provide a nonzero value" ValueError is
    ...
    # (): even when the user supplies
    # ``delta_var=`` to bypass the zero-range guard, the linfct slope
