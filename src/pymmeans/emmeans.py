...
def emmeans(
    ...
    specs
        ...
        (): also accepts R-style formula
        specs as a string — e.g. ``"pairwise ~ g"``, ``"trt.vs.ctrl ~
    ...
    def _is_cox_ph(raw_result: Any) -> bool:
        cls_chain = [raw_result.__class__.__name__]
        mdl = getattr(raw_result, "model", None)
        if mdl is not None:
            cls_chain.append(mdl.__class__.__name__)
        cox_marker_names = {"PHRegResults", "PHReg", "PHRegResultsWrapper"}
        return any(c in cox_marker_names for c in cls_chain)
    ...
    # (): R-style formula specs like
    # ``pairwise ~ g``, ``trt.vs.ctrl ~ g``, ``revpairwise ~ g | block``,
