...
def _response_to_link_emm(emm: Any) -> Any:
    ...
    promoted it to the canonical
    :func:`summary_layer._response_to_link_result` which handles both
    EMM and contrast frames. This wrapper preserves the original
    private name for callers inside `pwpm.py`.
    """
...
def pwpm(
    ...
    type
        ...
        **ratios** (matching R `emmeans::pwpm(em, type='response')`).
        (): the release
        refused pwpm on a response-scale EMM via the `pairs()` guard;
        this branch now reconstructs a link-scale EMM, runs pairs,
        regrids the contrast, and stitches the ratio matrix together.
