...
def _bootstrap_ci_case(
    ...
    info = obj.model_info
    # Pickling a ModelInfo replaces ``data`` with an empty DataFrame
    # placeholder (not ``None``), so the empty-data and pickled-data
    # cases were previously distinguishable only via the
    # ``info.raw_result is None`` side-channel below. After pickle the
    # zero-row check at the bottom of this block would fire with
    # "Cannot bootstrap a zero-row dataset.", which is a true
    # statement but hides the actual cause. Detect both shapes
    # explicitly so the steering message points at the real fix.
    if info.data is None or len(info.data) == 0:
        if refit_fn is None and info.raw_result is None:
            raise ValueError(
                "case bootstrap requires the original training data "
                "to draw resamples from, and the input ModelInfo has "
                "data=None or an empty DataFrame placeholder. The "
                "most common cause is that the EMM/contrast result "
                "was unpickled: pickling drops the original data and "
                "raw_result. Fixes:\n"
                ...
                "  - Supply ``refit_fn=callable`` explicitly (the "
                "callable receives a resampled DataFrame and must "
                "return a ModelInfo). When refit_fn= is given, the "
                "data must also be supplied via the callable."
            )
        # If refit_fn= was supplied we still need *some* data to
        # resample. Keeping the original behaviour for the
        # explicit-refit_fn path: the caller is expected to manage
        # the data themselves, so we fall through to the regular
        # error below if nothing was provided.
    data = info.data
    n_obs = len(data) if data is not None else 0
    if n_obs == 0:
        raise ValueError(
            "case bootstrap has no rows to resample. If the ModelInfo "
            "came from a pickle round-trip, re-fit in the current "
            "process or supply refit_fn= with the original data "
    ...
    if refit_fn is None:
        if info.raw_result is None:
            raise ValueError(
                ...
                "re-fit in the current process or pass refit_fn= "
                "explicitly."
            )
