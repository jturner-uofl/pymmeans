...
def bootstrap_ci(
    ...
    data
        Optional ``pandas.DataFrame`` to bootstrap from when the
        input EMM's ``model_info.data`` is unavailable (typically
        because the EMM was unpickled — pickling drops the original
        training DataFrame). Required if you supply ``refit_fn=``
        after a pickle round-trip; the refit callable receives a
        resampled view of ``data``. Ignored when
        ``info.data`` is already populated (the live-fit path
        resamples ``info.data`` and ``data=`` would silently
        contradict it).
...
def _bootstrap_ci_case(
    ...
    info = obj.model_info
    # Resolve the DataFrame to resample. The caller-supplied
    # ``data=`` kwarg wins when ``info.data`` is empty (the typical
    # post-pickle case). When both are populated, ``info.data``
    # wins — the live-fit path is the authoritative source, and
    # silently letting ``data=`` override it would let a caller
    # bootstrap against a *different* DataFrame than the one the
    # fit was actually conditioned on (a subtle wrong-numerics
    # footgun that this branch is specifically designed to avoid).
    has_info_data = info.data is not None and len(info.data) > 0
    if has_info_data:
        resample_df = info.data
    elif data is not None and len(data) > 0:
        resample_df = data
    else:
        raise ValueError(
            "case bootstrap requires a DataFrame to draw resamples "
            "from, and none is available. The input ModelInfo has "
            "data=None or an empty DataFrame placeholder (typical "
            "after a pickle round-trip — pickling drops the original "
            "training data and raw_result), and no ``data=`` kwarg "
            "was supplied to bootstrap_ci. Fixes:\n"
            "  - Run bootstrap_ci(em, kind='case', ...) BEFORE "
            "pickling, and pickle the bootstrap result instead.\n"
            "  - Re-fit in the current process and pass the fresh "
            "fit through emmeans() before calling bootstrap_ci.\n"
            "  - Supply both ``data=df`` AND ``refit_fn=callable`` "
            "to bootstrap_ci: ``data`` is the DataFrame to resample, "
            "``refit_fn(sample) -> fitted_result`` rebuilds the "
            "model on each resample. When the original DataFrame is "
            "preserved separately (e.g. on disk alongside the "
            "pickled EMM), this is the right escape hatch."
        )
    # Diagnostic: if both info.data AND the data= kwarg are
    # populated, warn the user that info.data wins. Silent override
    # of info.data by a caller-supplied data= would be confusing.
    if has_info_data and data is not None:
        import warnings as _w
        _w.warn(
            "bootstrap_ci(kind='case'): both model_info.data and "
            "data= are populated. Ignoring data= and resampling from "
            "model_info.data (the source the fit was actually "
            "conditioned on). Drop the data= kwarg to silence this "
            "warning.",
            UserWarning,
            stacklevel=2,
        )

    n_obs = len(resample_df)
    if n_obs == 0:
        raise ValueError(
            "case bootstrap has no rows to resample. The resolved "
            "DataFrame is empty; supply a non-empty ``data=`` kwarg "
            "or refit in the current process."
        )
    ...
    if refit_fn is None:
        if info.raw_result is None:
            raise ValueError(
                ...
                "raw_result is dropped by pickle round-trips; "
                "re-fit in the current process or supply both "
                "``data=df`` and ``refit_fn=callable`` to "
                "bootstrap_ci."
            )
        refit_fn = _default_refit_fn(info.raw_result)
    # Below this point everything reads ``resample_df`` (live or
    # caller-supplied) wherever the previous code read ``info.data``.
    data = resample_df
