...
def _is_phreg(model: Any) -> bool:
    """Module-qualified ``isinstance`` check for statsmodels Cox PH.

    A bare ``type(model).__name__ in {"PHReg", "PHRegResults"}``
    short-name match collides with any unrelated class — including a
    user's own ``class PHReg(...): ...`` proxy in an experiment
    notebook — and silently routes that proxy into the Cox-PH
    code paths (synthetic ``np.log(<endog>)`` response_name, ``df=inf``
    override). Anchor to the actual statsmodels types instead.

    Each name is imported in isolation so a missing
    ``PHRegResultsWrapper`` symbol on older statsmodels releases
    degrades that single wrapper detection rather than disabling
    the whole check.
    """
    ...
    if not cox_type_list:
        return False
    return isinstance(model, tuple(cox_type_list))


def _cox_response_name_override(model: Any, endog_name: str) -> str:
    ...
    The dispatch is module-qualified via :func:`_is_phreg` so an
    unrelated proxy class with the same short name doesn't get
    silently rewritten to ``np.log(<endog>)``.
    """
    if _is_phreg(model):
...
def from_statsmodels(result: Any) -> ModelInfo:
    ...
    if is_mixed:
        ...
        # naive "drop last k_extra entries" approach was therefore
        # silently wrong for ZIP: it kept the inflation parameter and
        # dropped a real structural coefficient.
        #
    ...
    if _is_phreg(model) or _is_phreg(result):
    ...
    return ModelInfo(
        ...
        design_info=design_info,
        data=frame,
        response_name=_cox_response_name_override(model, model.endog_names),
        family=_response_family_for_model(model),
        scale=float(getattr(result, "scale", 1.0) or 1.0),
        is_mixed=is_mixed,
        aliases=aliases,
