...
def _emmip_frame(
    ...
    (): mirrors R `emmip.emmGrid`'s
    ``plotit = FALSE`` return. R's column order is:
    ...
    helpers. (): the original factor
    columns are now retained (were dropped); row order is x-major
    (tension L, M, H — model's factor order), then by-major
    (wool A, B), matching R's `expand.grid` layout.
    """
    import numpy as np
    import pandas as pd
...
def emmip(
    ...
    CIs
        (): R-style alias for
        ``show_ci``. When supplied, overrides ``show_ci``. R's
        default is ``FALSE``; pymmeans keeps ``show_ci=True`` for
        backwards compatibility unless ``CIs`` is given.
    PIs
    ...
    plotit
        (): if ``False``, return the
        underlying ``pandas.DataFrame`` (with R-style columns
        ``xvar`` / ``yvar`` / ``SE`` / ``df`` / ``tvar`` / optional
        ``LCL`` / ``UCL`` / ``LPL`` / ``UPL``) instead of drawing
        the plot. Mirrors R ``emmip(..., plotit = FALSE)``. When
        ``plotit=False`` the R default of ``CIs=FALSE`` applies
        unless the caller explicitly passes ``CIs=True`` (this
        keeps the data-frame return columnar-stable for grep-style
        column matching from ported R code).
    """
    ...
    # Build a prediction-SE for the optional PI band.
    # (): match R `emmip.emmGrid(PIs=TRUE)` exactly:
    #
