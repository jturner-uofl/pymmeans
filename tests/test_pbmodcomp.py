...
@pytest.mark.filterwarnings(
    # pytest filterwarnings uses ``:`` as the field separator
    # (action:message:category:module:lineno), so the message regex
    # cannot itself contain an unescaped colon. Python's
    # ``warnings.filterwarnings(message=...)`` matches the regex
    # against the *start* of the warning message, so we anchor with
    # ``.*`` to skip past the literal ``pbmodcomp(): `` prefix and
    # then nail down the unique convergence-warning body.
    r"ignore:.*only \d+/\d+ bootstrap iterations:UserWarning"
)
def test_pbmodcomp_bootstrap_more_conservative_in_small_sample():
    ...
    filtered (with a message-anchored pattern so it only suppresses
    the convergence warning, not arbitrary other ``UserWarning``\\ s
    that might mask a real regression) so the test passes under
    ``pytest -W error``.
    """
