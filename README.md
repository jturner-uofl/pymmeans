...
Status: **Beta** · v0.21.0 · API stable, numerical surface frozen.
...
Beta. Version 0.21.0. API stable across the OLS, GLM, MixedLM, GEE, Cox
...
The public API surface is exercised by 607 unit tests.
...
| Line coverage                           | 87 %                           |
| Validation contracts                    | 347 (176 R cross-validation + 126 structural + 45 Monte-Carlo) |
...
- **Automated test suite.** 1,192 unit tests at 87 % line coverage
  (607 on the public API surface). Execute with `make test` or `pytest -q`.
- **Narrative validation notebook.** 347 enumerated contracts spanning
  direct cross-validation against R reference values (176), structural
  and self-consistency identities (126), and Monte-Carlo coverage and
