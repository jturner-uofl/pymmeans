...
Status: **Beta** · v0.15.0 · API stable, numerical surface frozen.
...
Beta. Version 0.15.0. API stable across the OLS, GLM, MixedLM, GEE, Cox
...
The public API surface is exercised by 562 unit tests.
The full suite reaches 87% line coverage and totals 1,168 passing
tests, including the internal audit-regression file.

| Metric                                  | Value                          |
|-----------------------------------------|--------------------------------|
| Unit tests                              | 1,168 passing (562 public-surface) |
| Line coverage                           | 87 %                           |
| Validation contracts                    | 329 (175 R cross-validation + 112 structural + 42 Monte-Carlo) |
...
- Direct cross-validation against R `emmeans` reference values to
  `atol = 1e-14` on the deterministic surface (175 contracts).
...
- **Automated test suite.** 1,168 unit tests at 87 % line coverage
  (562 on the public API surface). Execute with `make test` or `pytest -q`.
- **Narrative validation notebook.** 329 enumerated contracts spanning
  direct cross-validation against R reference values (175), structural
  and self-consistency identities (112), and Monte-Carlo coverage and
