...
Status: **Beta** · v0.12.0 · API stable, numerical surface frozen.
...
Beta. Version 0.12.0. API stable across the OLS, GLM, MixedLM, GEE, Cox
...
The public API surface is exercised by 536 unit tests.
The full suite reaches 87% line coverage and totals 1,142 passing
tests, including the internal audit-regression file.

| Metric                                  | Value                          |
|-----------------------------------------|--------------------------------|
| Unit tests                              | 1,142 passing (536 public-surface) |
| Line coverage                           | 87 %                           |
| Validation contracts                    | 315 (167 R cross-validation + 106 structural + 42 Monte-Carlo) |
...
- Direct cross-validation against R `emmeans` reference values to
  `atol = 1e-14` on the deterministic surface (167 contracts).
...
- **Automated test suite.** 1,142 unit tests at 87 % line coverage
  (536 on the public API surface). Execute with `make test` or `pytest -q`.
- **Narrative validation notebook.** 315 enumerated contracts spanning
  direct cross-validation against R reference values (167), structural
  and self-consistency identities (106), and Monte-Carlo coverage and
