...
> Status: **Beta** (v0.1.3). API stable across the OLS / GLM / MixedLM /
...
> 312 unit tests on the public surface (`pytest`), 54% line coverage. Minor
...
| `apply_satterthwaite(emm)` | Replace `df=∞` with Satterthwaite df (any RE structure) for `MixedLM`. |
| `apply_kenward_roger(emm)` | KR-inflated vcov + KR df. Cross-validated against `pbkrtest::KRmodcomp` (Kackar-Harville form). |
...
312 public-surface unit tests pass (1 skipped — `arviz` optional). 18 R-`emmeans` reference
...
|---|---|---|
| `emmeans` on n=1K OLS | 0.023 s | 0.002 s (~14× faster) |
| `emmeans` on n=500K OLS | 0.178 s | 0.023 s (~8× faster) |
