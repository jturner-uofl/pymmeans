...
Status: **Beta** · v0.13.0 · API stable, numerical surface frozen.
...
Beta. Version 0.13.0. API stable across the OLS, GLM, MixedLM, GEE, Cox
...
`re_formula`, and `vc_formula` random-effects syntaxes. 85 of 100
strict R-`emmeans` parity items (89 of 100 with partially-supported
...
The public API surface is exercised by 546 unit tests.
The full suite reaches 87% line coverage and totals 1,152 passing
tests, including the internal audit-regression file.

| Metric                                  | Value                          |
|-----------------------------------------|--------------------------------|
| Unit tests                              | 1,152 passing (546 public-surface) |
| Line coverage                           | 87 %                           |
| Validation contracts                    | 321 (172 R cross-validation + 107 structural + 42 Monte-Carlo) |
...
- Direct cross-validation against R `emmeans` reference values to
  `atol = 1e-14` on the deterministic surface (172 contracts).
...
- **Automated test suite.** 1,152 unit tests at 87 % line coverage
  (546 on the public API surface). Execute with `make test` or `pytest -q`.
- **Narrative validation notebook.** 321 enumerated contracts spanning
  direct cross-validation against R reference values (172), structural
  and self-consistency identities (107), and Monte-Carlo coverage and
