...
Status: **Beta** · v0.20.0 · API stable, numerical surface frozen.
...
Beta. Version 0.20.0. API stable across the OLS, GLM, MixedLM, GEE, Cox
...
The public API surface is exercised by 598 unit tests.
...
| Line coverage                           | 87 %                           |
| Validation contracts                    | 343 (176 R cross-validation + 122 structural + 45 Monte-Carlo) |
...
- **Automated test suite.** 1,192 unit tests at 87 % line coverage
  (598 on the public API surface). Execute with `make test` or `pytest -q`.
- **Narrative validation notebook.** 343 enumerated contracts spanning
  direct cross-validation against R reference values (176), structural
  and self-consistency identities (122), and Monte-Carlo coverage and
