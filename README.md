...
Status: **Beta** · v0.15.1 · API stable, numerical surface frozen.
...
Beta. Version 0.15.1. API stable across the OLS, GLM, MixedLM, GEE, Cox
...
The public API surface is exercised by 566 unit tests.
The full suite reaches 87% line coverage and totals 1,172 passing
tests, including the internal audit-regression file.

| Metric                                  | Value                          |
|-----------------------------------------|--------------------------------|
| Unit tests                              | 1,172 passing (566 public-surface) |
| Line coverage                           | 87 %                           |
| Validation contracts                    | 331 (176 R cross-validation + 113 structural + 42 Monte-Carlo) |
...
- **Automated test suite.** 1,172 unit tests at 87 % line coverage
  (566 on the public API surface). Execute with `make test` or `pytest -q`.
- **Narrative validation notebook.** 331 enumerated contracts spanning
  direct cross-validation against R reference values (176), structural
  and self-consistency identities (113), and Monte-Carlo coverage and
