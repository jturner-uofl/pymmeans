...
Status: **Beta** · v0.14.0 · API stable, numerical surface frozen.
...
Beta. Version 0.14.0. API stable across the OLS, GLM, MixedLM, GEE, Cox
...
The public API surface is exercised by 555 unit tests.
The full suite reaches 87% line coverage and totals 1,161 passing
tests, including the internal audit-regression file.

| Metric                                  | Value                          |
|-----------------------------------------|--------------------------------|
| Unit tests                              | 1,161 passing (555 public-surface) |
| Line coverage                           | 87 %                           |
| Validation contracts                    | 326 (173 R cross-validation + 111 structural + 42 Monte-Carlo) |
...
- Direct cross-validation against R `emmeans` reference values to
  `atol = 1e-14` on the deterministic surface (173 contracts).
...
- **Automated test suite.** 1,161 unit tests at 87 % line coverage
  (555 on the public API surface). Execute with `make test` or `pytest -q`.
- **Narrative validation notebook.** 326 enumerated contracts spanning
  direct cross-validation against R reference values (173), structural
  and self-consistency identities (111), and Monte-Carlo coverage and
